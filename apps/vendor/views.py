...
from .models import Vendor,OepeningHour
...
def opening_hours(request):
    opening_hours = OepeningHour.objects.filter(vendor=get_vendor(request))
...
def add_opening_hours(request):
    ...
    if request.user.is_authenticated:
        ...
        try:
            hour = OepeningHour.objects.create(vendor=get_vendor(request),day=day,from_hour=from_hour,to_hour=to_hour,is_closed=is_closed)
            if hour:
                day = OepeningHour.objects.get(id=hour.id)
                if day.is_closed:
